...
const Visualizer: React.FC<VisualizerProps> = ({ config, setConfig, gestureState, aiPositions }) => {
  ...
  useEffect(() => {
    ...
    folder.add(config, 'diffusionStrength', 0, 5.0).name('3D立体扩散');
    folder.add(config, 'collisionStrength', 0, 50.0).name('碰撞排斥力');
    folder.add(config, 'randomScatter', 0, 100.0).name('随机扩散偏移');
    folder.add(config, 'morphSpeed', 0.01, 0.2).name('变形重组速度');
    
    const shapeControl = folder.add(config, 'activeShape', Object.values(ShapeType)).name('粒子形态集');
    shapeControl.onChange(v => {
      if (v !== ShapeType.CUSTOM_IMAGE && v !== ShapeType.AI_PROMPT) {
        targetPositionsRef.current = generateShapePositions(v as ShapeType);
        targetColorsRef.current = null;
        setConfig(prev => ({ ...prev, activeShape: v }));
      }
    });
    ...
    folder.add(uploadBtn, 'upload').name('📷 上传图片生成模型');
    folder.add(config, 'autoRotate').name('自适应旋转');
    folder.open();
    ...
    const animate = () => {
      ...
      if (pointsRef.current && geometryRef.current) {
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
          ...
          if (config.diffusionStrength > 0) {
            const diff = stats.bass * config.diffusionStrength * 60;
            positions[i3] += (px / dist) * diff;
            positions[i3+1] += (py / dist) * diff;
            positions[i3+2] += (pz / dist) * diff;
          }

          if (currentExplosion > 0.05) {
            const ef = currentExplosion * 40;
            positions[i3] += (px / dist) * ef + Math.sin(i * 0.2) * ef * 0.5;
            positions[i3+1] += (py / dist) * ef + Math.cos(i * 0.2) * ef * 0.5;
            positions[i3+2] += (pz / dist) * ef + Math.sin(i * 0.25) * ef * 0.5;
          }

          const jitter = (config.randomScatter * 0.15) + (stats.bass * 4.0);
